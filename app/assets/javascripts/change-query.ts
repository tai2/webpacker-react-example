...
function prepereSelectElems(): void {
  const doms = document.querySelectorAll('select[data-change-query]') as NodeListOf<HTMLSelectElement>
...
function prepereCheckboxElems(): void {
  const doms = document.querySelectorAll('input[type=checkbox][data-change-query]') as NodeListOf<HTMLInputElement>
  const query = queryString.parse(location.search)
